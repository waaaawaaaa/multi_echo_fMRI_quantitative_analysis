...
% ID={'016','017','018','019','020','021','022','023','024','025','026','027','029','030','031','032'};
% ID={'001','002','003','004','005','006','007','010','011','013','015'};
...
%         t2starFIT.img=uint16(t2s * 8000 + 10);     %将较小的值放大
        t2starFIT.img=uint16(t2s * 8000);     %+10会导致算tSNR时边边上本来为0的值，除以标准差之后就变成了巨大，TSNR不对
        t2starFIT.hdr = nii_echo1.hdr;
%         save_nii(t2starFIT,[save_path '\t2starFIT.nii'])%保存为.nii文件
        save_nii(t2starFIT,[save_path '\t2starFIT_no10.nii'])%保存为.nii文件
