...
% jobfile = {'D:\proprecess\sub-001\step3_2_job.m'};
...
function matlabbatch = step32(file_root,ID,task,combined_data)
    ...
    save_path=[file_root 'sub-' ID '\' task '\1st_' combined_data];
    physio_files = [file_root 'sub-' ID '\' task '\physio\multiple_regressors.txt'];
    mkdir(save_path)
    ...
    matlabbatch{8}.spm.stats.results.export{2}.binary.basename = 'report';
end