...
% file_root = 'F:\me_fmri_t2\';
task={'fingerTapping'};    %后面均写成了task{1}
combined_data={'t2starFIT','TE_combined','echo2','t2s_relax'};
...
xlswrite(excel_t_test,{'sub','echo2','TEcombined','T2*FIT','t2*relax'},1,'A1');
xlswrite(excel_t_test,ID',1,'A2');    %竖着写一行ID

xlswrite(excel_PSC,{'sub','echo2','TEcombined','T2*FIT','t2*relax'},1,'A1');
...
xlswrite(excel_functional_contrast,{'sub','echo2','TEcombined','T2*FIT','t2*relax'},1,'A1');
xlswrite(excel_functional_contrast,ID',1,'A2');

xlswrite(excel_tCNR,{'sub','echo2','TEcombined','T2*FIT','t2*relax'},1,'A1');
...
%     BW_t2starFIT = imbinarize(tmap_t2starFIT,0);
    ...
    tmap_t2srelax_dir = [file_dir combined_data{4} '\spmT_0001_report.nii'];
    tmap_file_t2srelax = load_nii(tmap_t2srelax_dir );
    tmap_t2srelax = double(tmap_file_t2srelax.img);
%     BW_t2starFIT = imbinarize(tmap_t2starFIT,0);

    [Ni, Nj, Nk] = size(tmap_echo2);
    ROI_FWE_OR=tmap_echo2| tmap_TEcom| tmap_t2starFIT| tmap_t2srelax;   %求逻辑或
...
%     Resliced_ROI = double(Resliced_ROI_file.img);    %读入重采样好的ROI
    ...
    tmap_t2srelax_dir = [file_dir combined_data{4} '\spmT_0001.nii'];
    mean_tvalue_t2srelax = calculate_t_value(tmap_t2srelax_dir,Resliced_ROI);
    xlswrite(excel_t_test,mean_tvalue_t2srelax,1,['E' num2str(i+1)]);
    ...
    bata_dir_t2srelax =  [file_dir combined_data{4}];
    mean_PSC_t2srelax = calculate_PSC(bata_dir_t2srelax,Resliced_ROI);
    xlswrite(excel_PSC,mean_PSC_t2srelax,1,['E' num2str(i+1)]);
...
%     mean_tCNR_t2starFIT = calculate_tCNR(realtime_tPSC_t2starFIT,mean_functional_contrast_t2starFIT);
    ...
    xlswrite(excel_tPSC,realtime_tPSC_t2srelax,1,['C' num2str(82+i)]);

%     mean_functional_contrast_echo2 = calculate_functional_contrast(realtime_tPSC_echo2,contrast_vector);
    xlswrite(excel_functional_contrast,mean_cnr_t2srelax,1,['E' num2str(i+1)]);
%     mean_tCNR_echo2 = calculate_tCNR(realtime_tPSC_echo2,mean_functional_contrast_echo2);
    xlswrite(excel_tCNR,mean_tCNR_t2srelax,1,['E' num2str(i+1)]);
...
% 
    ...
    cumulativemean=zeros(size(ROI_data_2D));
    psc_data_cumulativemean=zeros(size(ROI_data_2D));
...
%         % Cumulative
       cumulativemean(i,:) = nanmean(data_2D_detrended,1); %[1, voxels/signals]
...
%     psc_timeseries=mean(psc_data_cumulativebaselinemean');
    psc_timeseries=mean(psc_data_cumulativemean');
%     for i=2:209
%         psc_timeseries(i)=(psc_timeseries(i-1)+psc_timeseries(i)+psc_timeseries(i+1))/3;
%     end
%     psc_timeseries(1:2)=[0,0];

    cnr = nanmean(psc_data_cumulativemean(I_taskPSC,:), 1) - nanmean(psc_data_cumulativemean(I_baselinePSC, :), 1);
    tcnr = cnr./std(psc_data_cumulativemean,0,1);    %结果基本没差
    mean_cnr=mean(cnr);
    mean_tcnr=mean(tcnr);
%     mean_cnr = nanmean(psc_timeseries(I_taskPSC,:),1) - nanmean(psc_timeseries(I_baselinePSC,:),1);
%     mean_tcnr = mean_cnr/std(psc_timeseries,0,1);
