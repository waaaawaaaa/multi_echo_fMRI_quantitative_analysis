...
% combined_data={'t2s_relax','t2starFIT','TE_combined','echo2'};
combined_data={'t2s_relax','t2starFIT'};
...
xls_dir = [file_root 'result_relax_fit'];    %保存路径
mkdir(xls_dir);
Ni = 45;Nj = 55;Nk = 45;
...
xlswrite(excel_t_test,{'sub','T2*FIT','t2*relax'},1,'A1');
xlswrite(excel_t_test,ID',1,'A2');    %竖着写一行ID

xlswrite(excel_PSC,{'sub','T2*FIT','t2*relax'},1,'A1');
xlswrite(excel_PSC,ID',1,'A2');

xlswrite(excel_tPSC,{'type','T2*FIT'}',1,'A1');
xlswrite(excel_tPSC,{'t2*relax'}',1,'A29');  %27个受试者
% xlswrite(excel_tPSC,{'T2*FIT'}',1,'A56');
% xlswrite(excel_tPSC,{'t2*relax'}',1,'A83');
xlswrite(excel_tPSC,{'sub'},1,'B1');
xlswrite(excel_tPSC,ID',1,'B2');
xlswrite(excel_tPSC,ID',1,'B29');
% xlswrite(excel_tPSC,ID',1,'B56');
% xlswrite(excel_tPSC,ID',1,'B83');
...
xlswrite(excel_functional_contrast,{'sub','T2*FIT','t2*relax'},1,'A1');
xlswrite(excel_functional_contrast,ID',1,'A2');

xlswrite(excel_tCNR,{'sub','T2*FIT','t2*relax'},1,'A1');
...
% %     BW_echo2 = imbinarize(tmap_echo2,0);
    ...
    tmap_t2srelax_dir = [file_dir combined_data{1} '\spmT_0001_report.nii'];
    tmap_file_t2srelax = load_nii(tmap_t2srelax_dir );
    tmap_t2srelax = double(tmap_file_t2srelax.img);
%     BW_TEcom = imbinarize(tmap_TEcom,0);

    tmap_t2starFIT_dir = [file_dir combined_data{2} '\spmT_0001_report.nii'];
...
%     BW_t2starFIT = imbinarize(tmap_t2starFIT,0);
    ...
    [Ni, Nj, Nk] = size(tmap_t2starFIT);
    ROI_FWE_OR=tmap_t2srelax| tmap_t2starFIT;   %求逻辑或
...
%     end
    ...
    FWE_OR_nii.hdr = tmap_file_t2starFIT.hdr;
...
%     Resliced_ROI = double(Resliced_ROI_file.img);    %读入重采样好的ROI
    ...
    tmap_t2srelax_dir = [file_dir combined_data{1} '\spmT_0001.nii'];
    mean_tvalue_t2srelax = calculate_t_value(tmap_t2srelax_dir,Resliced_ROI);
    xlswrite(excel_t_test,mean_tvalue_t2srelax,1,['B' num2str(i+1)]);
%     tmap_t2srelax_dir = [file_dir combined_data{2} '\spmT_0001.nii'];
%     mean_tvalue_TEcom = calculate_t_value(tmap_t2srelax_dir,Resliced_ROI);
%     xlswrite(excel_t_test,mean_tvalue_TEcom,1,['C' num2str(i+1)]);
    tmap_t2starFIT_dir = [file_dir combined_data{2} '\spmT_0001.nii'];
    mean_tvalue_t2starFIT = calculate_t_value(tmap_t2starFIT_dir,Resliced_ROI);
    xlswrite(excel_t_test,mean_tvalue_t2starFIT,1,['C' num2str(i+1)]);

    % 4.2 calculate and save PSC file 

    bata_dir_t2srelax =  [file_dir combined_data{1}];
    mean_PSC_t2srelax = calculate_PSC(bata_dir_t2srelax,Resliced_ROI);
    xlswrite(excel_PSC,mean_PSC_t2srelax,1,['B' num2str(i+1)]);
...
%     xlswrite(excel_PSC,mean_PSC_TEcom,1,['C' num2str(i+1)]);
    ...
    beta_dir_t2s = [file_dir combined_data{2}];
    mean_PSC_t2s = calculate_PSC(beta_dir_t2s,Resliced_ROI);
    xlswrite(excel_PSC,mean_PSC_t2s,1,['C' num2str(i+1)]);
    disp(['Finished PSC Calaulated : ' ID{i} ]);

    % 4.3 calculate realtime tPSC and tCNR
    
    smoothed_dir_t2srelax = [file_root 'sub-' ID{i} '\' task{1} '\swt2s_relax.nii'];    %平滑后的影像
    [realtime_tPSC_t2srelax,mean_cnr_t2srelax,mean_tCNR_t2srelax] = calculate_realtime_tPSC(smoothed_dir_t2srelax,Resliced_ROI);
    xlswrite(excel_tPSC,realtime_tPSC_t2srelax,1,['C' num2str(1+i)]);

%     mean_functional_contrast_echo2 = calculate_functional_contrast(realtime_tPSC_echo2,contrast_vector);
    xlswrite(excel_functional_contrast,mean_cnr_t2srelax,1,['B' num2str(i+1)]);
%     mean_tCNR_echo2 = calculate_tCNR(realtime_tPSC_echo2,mean_functional_contrast_echo2);
    xlswrite(excel_tCNR,mean_tCNR_t2srelax,1,['B' num2str(i+1)]);
...
%     xlswrite(excel_tCNR,mean_tCNR_TE_com,1,['C' num2str(i+1)]);
    ...
    xlswrite(excel_tPSC,realtime_tPSC_t2starFIT,1,['C' num2str(28+i)]);

%     mean_functional_contrast_t2starFIT = calculate_functional_contrast(realtime_tPSC_t2starFIT,contrast_vector);
    xlswrite(excel_functional_contrast,mean_functional_contrast_t2starFIT,1,['C' num2str(i+1)]);
%     mean_tCNR_t2starFIT = calculate_tCNR(realtime_tPSC_t2starFIT,mean_functional_contrast_t2starFIT);
    xlswrite(excel_tCNR,mean_tCNR_t2starFIT,1,['C' num2str(i+1)]);
