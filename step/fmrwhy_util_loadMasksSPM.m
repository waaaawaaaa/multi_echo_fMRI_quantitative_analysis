...
%     preproc_dir = fullfile(deriv_dir, 'fmrwhy-preproc');
    sub_dir_preproc = fullfile(bids_dir, ['sub-' sub]);

    masks = struct;

    masks.GM_mask_fn = fullfile(sub_dir_preproc, 'anat', ['rc1sub-' sub '_T1w.nii']);   %c1:GM; c2:WM； c3:CSF
    masks.WM_mask_fn = fullfile(sub_dir_preproc, 'anat', ['rc2sub-' sub '_T1w.nii']);
    masks.CSF_mask_fn = fullfile(sub_dir_preproc, 'anat', ['rc3sub-' sub '_T1w.nii']);
    ...
    %全脑是指前三个的逻辑或
    [Ni, Nj, Nk] = size(masks.GM_mask_3D);
    masks.brain_mask_3D=ones(Ni, Nj, Nk);
    for x=1:Ni
        for y=1:Nj
            for k=1:Nk
                if masks.GM_mask_3D(x,y,k)==0 && masks.WM_mask_3D(x,y,k)==0 && masks.CSF_mask_3D(x,y,k)==0
                    masks.brain_mask_3D(x,y,k)=0;
                end
            end
        end
    end
